...
-- Drop tables
-- -----------------------------------------------------
DROP TABLE IF EXISTS `GusFoot`.`jogo_andamento`;
DROP TABLE IF EXISTS `GusFoot`.`classificacao`;
DROP TABLE IF EXISTS `GusFoot`.`servidor_usuario`;
DROP TABLE IF EXISTS `GusFoot`.`tabela_jogos`;
DROP TABLE IF EXISTS `GusFoot`.`liga`;
DROP TABLE IF EXISTS `GusFoot`.`jogador`;
DROP TABLE IF EXISTS `GusFoot`.`time`;
DROP TABLE IF EXISTS `GusFoot`.`servidor`;
DROP TABLE IF EXISTS `GusFoot`.`usuario`;
DROP TABLE IF EXISTS `GusFoot`.`jogador_base`;
DROP TABLE IF EXISTS `GusFoot`.`time_base`;
DROP TABLE IF EXISTS `GusFoot`.`posicao`;
DROP TABLE IF EXISTS `GusFoot`.`pais`;
...
-- -----------------------------------------------------
-- Table `GusFoot`.`time_base`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `GusFoot`.`time_base` (
  `id_time_base` INT NOT NULL AUTO_INCREMENT,
  `id_pais` INT NOT NULL,
  `nome` VARCHAR(50) NOT NULL,
  `sigla` VARCHAR(3) NOT NULL,
  `escudo` VARCHAR(50) NULL,
  PRIMARY KEY (`id_time_base`),
  CONSTRAINT `fk_time_base_pais1`
    FOREIGN KEY (`id_pais`)
...
CREATE INDEX `fk_time_base_pais1_idx` ON `GusFoot`.`time_base` (`id_pais` ASC) ;
...
CREATE TABLE IF NOT EXISTS `GusFoot`.`jogador_base` (
  ...
  CONSTRAINT `fk_jogador_base_posicao1`
    ...
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_jogador_base_time_base1`
    FOREIGN KEY (`id_time_base`)
    REFERENCES `GusFoot`.`time_base` (`id_time_base`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB;

CREATE INDEX `fk_jogador_base_pais1_idx` ON `GusFoot`.`jogador_base` (`id_pais` ASC) ;

CREATE INDEX `fk_jogador_base_posicao1_idx` ON `GusFoot`.`jogador_base` (`id_posicao` ASC) ;

CREATE INDEX `fk_jogador_base_time_base1_idx` ON `GusFoot`.`jogador_base` (`id_time_base` ASC) ;
